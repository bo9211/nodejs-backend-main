...
router.post('/password-reset-link', async (req, res) => {
  ...
  // todo: write your code here
    ...
    if (!user) {
      // If no user found, respond with an error or a generic message to avoid email enumeration
      return res.status(404).send({ message: 'If a user with that email exists, a reset link will be sent.' });
    }
...
// Continue with the rest of your code for the password-reset/confirm route...
